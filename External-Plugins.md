...
> CHART type.id name title units [family [category [charttype [priority [update_every]]]]]
 ...
 where:
  ...
  - `name`
    ...
    is the name that will be presented to the user instead of `id` in `type.id`. This means that only the `id` part of `type.id` is changed. When a name has been given, the chart is index (and can be referred) as both `type.id` and `type.name`.
